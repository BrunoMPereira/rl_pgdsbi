...
def get_stock_data(companies,column_to_keep='Close'):
    output = pd.DataFrame(columns=['Date'])
    is_first = True
    for company in companies:
        df = pd.read_csv(f'data/{company}.csv')
        df = df[['Date',column_to_keep]]
        df = df.rename(columns = {column_to_keep: company})
        if is_first:
            output = df.copy()
            is_first = False
        else:
...
class market:
    def __init__(self, companies, budget=1e4,column_to_keep='Close'):
        self.data = get_stock_data(companies,column_to_keep)
        self.budget = budget
