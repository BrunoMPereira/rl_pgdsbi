...
def get_stock_data(companies):
    ...
    for company in companies:
        ...
        # Fiability score - a percentage of close value
        # The idea is that a closeness of the adjusted
        df['Perc_Metric'] = df['Adjusted Close'] / df['Close']
        df = df[['Date','Perc_Metric']]
        df = df.rename(columns = {'Perc_Metric': company})
        if is_first:
            output = df.copy()
            is_first = False
        else:
